...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		ArrayList<Integer> hop2Vertices =new ArrayList<Integer>();
		//Normal method take the one hop veritces of the vetex, find one hop vertices of each in those set.
		/*
		ArrayList<Integer> hop1Vertices =new ArrayList<Integer>();
		hop1Vertices = (ArrayList<Integer>) this.getNeighbors(v);
		for(Integer vtx : hop1Vertices)
		{
			hop2Vertices.addAll(this.getNeighbors(vtx));
		}*/
		
		//looping through the vertex and adding to the list of 2 hop vertices,  applying the logic.
		int n = this.getNumVertices();
		for(int j=0;j<n;j++)
		{
			if(this.adjMatrix[v][j]>0) {
				for(int k=0;k<n;k++)
				{
					if(this.adjMatrix[j][k]>0)
					{
						hop2Vertices.add(k);
					}
				}
		...
		for(int i=0;i<n;i++) {
			for(int j=0;j<n;j++) {
				for(int k=0;k<n;k++)
				{
					resultMatrix[i][j] += this.adjMatrix[i][k]* this.adjMatrix[k][j];
				}
			}
		}
		for(int i=0; i< n; i++)
		{
			if(resultMatrix[v][i]>0)
			{
				for(int k=0; k< resMatrix[v][i];k++)
				{
					hop2Vertices.add(i);
				}
			}
